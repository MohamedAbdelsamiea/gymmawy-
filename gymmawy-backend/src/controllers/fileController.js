...
const PROGRAMMES_DIR = path.join(UPLOADS_DIR, 'programmes');
...
/**
 * Secure route for serving programme PDF files (Admin only)
 * GET /files/programmes/:filename
 */
router.get('/programmes/:filename', requireAuth, async (req, res) => {
  try {
    const { filename } = req.params;
    const userRole = req.user.role;

    console.log('🔍 Programme PDF request:', {
      filename,
      userRole,
      userAgent: req.headers['user-agent'],
      origin: req.headers.origin
    ...
    // Security: Double-check the resolved path is within the programmes directory
    if (!filePath.startsWith(PROGRAMMES_DIR)) {
      return res.status(400).json({ 
        error: { message: "Invalid file path" } 
      });
    }
    ...
    try {
      ...
      if (error.code === 'ENOENT') {
        console.log('❌ Programme PDF not found:', filename);
        return res.status(404).json({ 
          error: { message: "File not found" } 
        });
      }
    ...
    res.sendFile(filePath, (error) => {
      if (error) {
        console.error('Error serving programme PDF:', error);
        if (!res.headersSent) {
          res.status(500).json({ 
            error: { message: "Error serving file" } 
          });
    ...
    console.error('Programme PDF file access error:', error);
